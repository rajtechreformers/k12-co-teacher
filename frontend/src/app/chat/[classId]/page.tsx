...
export default function ChatPage() {
  ...
  const { isConnected, connect, disconnect, sendMessage } = useWebSocket({
    ...
    onError: (error) => {
      ...
      console.error('WebSocket error in ChatPage:', errorDetails);
    },
    onClose: (event) => {
    ...
    const fetchClassData = async () => {
      ...
      try {
        ...
        if (cachedStudents) {
          ...
          // Fetch and cache students for this specific class
  ...
  const handleSendMessage = () => {
    if (message.trim() && isConnected) {
      ...
      console.log('Current sessionId before sending message:', sessionId);
      
      // Send to WebSocket with all student IDs for general chat
      const payload = {
        message: message.trim(),
  ...
  const handleChatClick = async (chat: ChatHistory) => {
    ...
    } else if (chat.type === "student" && chat.studentId) {
      router.push(`/chat/${classId}/student/${chat.studentId}`);
    }
  };
            ...
            {loading ? (
              ...
              chatHistory.map((chat) => (
                <motion.div
                  ...
                  className={`p-3 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors ${sessionId === chat.conversationId ? 'bg-blue-50 border border-blue-200' : ''}`}
                >
        ...
        <div className="bg-white/90 backdrop-blur-xl border-b border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className={`p-2 bg-gradient-to-br ${classInfo?.color || 'from-blue-500 to-indigo-600'} rounded-lg`}>
            ...
            <div className="space-y-3">
              {Object.entries(students).map(([studentId, studentName]) => (
                <motion.button
                  ...
                  className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors w-full text-left"
                >
